...
const WasteManagementMap = () => {
  ...
  const handleGarbageDump = async () => {
    ...
    if (weight !== '') {
      const storage =
        currentUser.role === 'House keeper' || currentPoint.type === 'Collection Point'
    ...
    <div className='h-96'>
        ...
        >
          {currentPoint && (
            <InfoWindowF position={currentPoint}>
              <div className='p-4'>
                <h1>Current Storage: {currentPoint.storage} kg</h1>
              </div>
            </InfoWindowF>
          )}
          {destination && (
          ...
          {currentUser.role === 'Driver' && (
            <MarkerF
              onDragEnd={(e) => setCurrentLocation({ latitude: e.latLng.lat(), longitude: e.latLng.lng() })}
              draggable
              icon={carMarker}
              position={center}
            />
          ...
          {markerLocations
            .filter((m) => (currentUser.role === 'House keeper' ? m.type === 'Dustbin' : true))
            .map((e) => (
              <MarkerF
                key={e.lng + e.id}
                icon={e.type === 'Dustbin' ? dustBinMarker : collectionMarker}
                onClick={() => setCurrentPoint(e)}
                onDblClick={() => currentUser.role === 'Driver' && setDestination(e)}
                position={e}
              />
            ))}
        </GoogleMap>
      ...
      {currentUser.role === 'Admin' && (
        <div className='flex flex-col items-center mt-4'>
          <label className='block text-gray-700 text-sm font-bold mb-2' htmlFor='markerType'>
            Select Marker Type
          </label>
          <select
      ...
      {currentUser.role !== 'Admin' && currentPoint && (
        <div className='flex flex-col items-center mt-4'>
          <label className='block text-gray-700 text-sm font-bold mb-2' htmlFor='garbageWeight'>
            Garbage Weight
          </label>
          <input
            required
          ...
          <div className='flex items-center justify-between'>
            ...
            >
              {currentUser.role === 'House keeper' || currentPoint.type === 'Collection Point' ? 'Dump' : 'Remove'}
            </button>
          </div>
        </div>
      )}
